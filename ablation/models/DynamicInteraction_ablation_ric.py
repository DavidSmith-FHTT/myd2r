...
from models.Cells import IntraModelReasoningCell, CrossModalRefinementCell, GlobalLocalAlignmentCell, ContextRichCrossModalCell, GlobalEnhancedSemanticCell
...
class DynamicInteraction_Layer0(nn.Module):
    ...
    消融实验：移除RectifiedIdentityCell (SSSR单元)
    """
    def __init__(self, args, num_cell, num_out_path):
        ...
        # 移除 self.ric = RectifiedIdentityCell(args, num_out_path)
    ...
    def forward(self, text, image):
        ...
        # 只通过五个处理单元处理输入（移除RectifiedIdentityCell）
        path_prob = [None] * self.num_cell
        emb_lst = [None] * self.num_cell
        # 移除: emb_lst[0], path_prob[0] = self.ric(text)
        emb_lst[0], path_prob[0] = self.glac(text, image)
        ...
        for i in range(self.num_out_path):
            skip_emb = unsqueeze2d(gate_mask[:, i]) * emb_lst[0]  # 使用glac作为skip
            res = 0
...
class DynamicInteraction_Layer(nn.Module):
    ...
    def forward(self, ref_wrd, text, image):
        ...
        # 通过5个不同的处理单元并行处理输入数据（移除RectifiedIdentityCell）
        path_prob = [None] * self.num_cell
        emb_lst = [None] * self.num_cell
        # 移除: emb_lst[0], path_prob[0] = self.ric(ref_wrd[0])
...
class Reversed_DynamicInteraction_Layer0(nn.Module):
    ...
    def forward(self, text, image):
        ...
        path_prob = [None] * self.num_cell
        emb_lst = [None] * self.num_cell
        # 移除: emb_lst[0], path_prob[0] = self.ric(image)
        emb_lst[0], path_prob[0] = self.glac(image, text)
...
class Reversed_DynamicInteraction_Layer(nn.Module):
    ...
    def forward(self, ref_wrd, text, image):
        ...
        emb_lst = [None] * self.num_cell
        # 移除: emb_lst[0], path_prob[0] = self.ric(ref_wrd[0])
        emb_lst[0], path_prob[0] = self.glac(ref_wrd[0], text)
        emb_lst[1], path_prob[1] = self.imrc(ref_wrd[1])
