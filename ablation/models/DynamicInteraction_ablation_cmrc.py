...
from models.Cells import RectifiedIdentityCell, IntraModelReasoningCell, GlobalLocalAlignmentCell, ContextRichCrossModalCell, GlobalEnhancedSemanticCell
...
class DynamicInteraction_Layer0(nn.Module):
    ...
    消融实验：移除CrossModalRefinementCell (CLSSM单元)
    """
    ...
    def forward(self, text, image):
        ...
        # 移除: emb_lst[3], path_prob[3] = self.cmrc(text, image)
...
class DynamicInteraction_Layer(nn.Module):
    ...
    def forward(self, ref_wrd, text, image):
        ...
        # 通过5个不同的处理单元并行处理输入数据（移除CrossModalRefinementCell）
        path_prob = [None] * self.num_cell
        emb_lst = [None] * self.num_cell
        ...
        # 移除: emb_lst[3], path_prob[3] = self.cmrc(ref_wrd[3], image)
...
class Reversed_DynamicInteraction_Layer0(nn.Module):
    ...
    def forward(self, text, image):
        ...
        # 移除: emb_lst[3], path_prob[3] = self.cmrc(image, text)
...
class Reversed_DynamicInteraction_Layer(nn.Module):
    ...
    def forward(self, ref_wrd, text, image):
        ...
        emb_lst[2], path_prob[2] = self.imrc(ref_wrd[2])
        # 移除: emb_lst[3], path_prob[3] = self.cmrc(ref_wrd[3], text)
