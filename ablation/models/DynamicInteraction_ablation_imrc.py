...
from models.Cells import RectifiedIdentityCell, CrossModalRefinementCell, GlobalLocalAlignmentCell, ContextRichCrossModalCell, GlobalEnhancedSemanticCell
...
class DynamicInteraction_Layer0(nn.Module):
    ...
    消融实验：移除IntraModelReasoningCell (USSR单元)
    """
    def __init__(self, args, num_cell, num_out_path):
        ...
        # 移除 self.imrc = IntraModelReasoningCell(args, num_out_path)
    ...
    def forward(self, text, image):
        ...
        # 只通过五个处理单元处理输入（移除IntraModelReasoningCell）
        ...
        # 移除: emb_lst[2], path_prob[2] = self.imrc(text)
...
class DynamicInteraction_Layer(nn.Module):
    ...
    def forward(self, ref_wrd, text, image):
        ...
        # 通过5个不同的处理单元并行处理输入数据（移除IntraModelReasoningCell）
        path_prob = [None] * self.num_cell
        emb_lst = [None] * self.num_cell
        emb_lst[0], path_prob[0] = self.ric(ref_wrd[0])
        emb_lst[1], path_prob[1] = self.glac(ref_wrd[1], image)
        # 移除: emb_lst[2], path_prob[2] = self.imrc(ref_wrd[2])
...
class Reversed_DynamicInteraction_Layer0(nn.Module):
    ...
    def forward(self, text, image):
        ...
        # 移除: emb_lst[2], path_prob[2] = self.imrc(image)
