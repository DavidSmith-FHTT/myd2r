...
from models.Cells import RectifiedIdentityCell, IntraModelReasoningCell, CrossModalRefinementCell, GlobalLocalAlignmentCell, ContextRichCrossModalCell
...
class DynamicInteraction_Layer0(nn.Module):
    ...
    消融实验：移除GlobalEnhancedSemanticCell (CGSSA单元)
    """
    def __init__(self, args, num_cell, num_out_path):
        ...
        # 移除 self.gesc = GlobalEnhancedSemanticCell(args, num_out_path)

    def forward(self, text, image):

        # 只通过五个处理单元处理输入（移除GlobalEnhancedSemanticCell）
        ...
        # 移除: emb_lst[5], path_prob[5] = self.gesc(text, image)
...
class DynamicInteraction_Layer(nn.Module):
    ...
    def forward(self, ref_wrd, text, image):
        ...
        # 通过5个不同的处理单元并行处理输入数据（移除GlobalEnhancedSemanticCell）
        path_prob = [None] * self.num_cell
        emb_lst = [None] * self.num_cell
        ...
        # 移除: emb_lst[5], path_prob[5] = self.gesc(ref_wrd[5], image)
...
class Reversed_DynamicInteraction_Layer0(nn.Module):
    ...
    def forward(self, text, image):
        ...
        # 移除: emb_lst[5], path_prob[5] = self.gesc(image, text)
...
class Reversed_DynamicInteraction_Layer(nn.Module):
    ...
    def forward(self, ref_wrd, text, image):
        ...
        # 移除: emb_lst[5], path_prob[5] = self.gesc(ref_wrd[5], text)
