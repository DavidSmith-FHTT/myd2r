...
from models.XModules import js_div
...
class UnimoModel(nn.Module):
    def __init__(self, args, vision_config, text_config, add_pooling_layer=True, num_self_layer=1):
        ...
        # 消融实验：使用简单拼接替代block_fusion
        # 拼接后维度为768*2=1536，通过线性层映射回768
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None,
        ...
        # 消融实验：简单拼接替代block_fusion
        concat_output = torch.cat([text_pooled_output, image_pooled_output], dim=-1)  # (B, 1536)
        output = self.concat_fusion(concat_output)  # (B, 768)
