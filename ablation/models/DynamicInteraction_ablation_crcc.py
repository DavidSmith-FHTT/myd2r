...
from models.Cells import RectifiedIdentityCell, IntraModelReasoningCell, CrossModalRefinementCell, GlobalLocalAlignmentCell, GlobalEnhancedSemanticCell
...
class DynamicInteraction_Layer0(nn.Module):
    ...
    消融实验：移除ContextRichCrossModalCell (MVSSU单元)
    """
    def __init__(self, args, num_cell, num_out_path):
        ...
        # 移除 self.crcmc = ContextRichCrossModalCell(args, num_out_path)
        self.gesc = GlobalEnhancedSemanticCell(args, num_out_path)

    def forward(self, text, image):

        # 只通过五个处理单元处理输入（移除ContextRichCrossModalCell）
        ...
        # 移除: emb_lst[4], path_prob[4] = self.crcmc(text, image)
...
class DynamicInteraction_Layer(nn.Module):
    ...
    def forward(self, ref_wrd, text, image):
        ...
        # 通过5个不同的处理单元并行处理输入数据（移除ContextRichCrossModalCell）
        path_prob = [None] * self.num_cell
        emb_lst = [None] * self.num_cell
        ...
        # 移除: emb_lst[4], path_prob[4] = self.crcmc(ref_wrd[4], image)
...
class Reversed_DynamicInteraction_Layer0(nn.Module):
    ...
    def forward(self, text, image):
        ...
        # 移除: emb_lst[4], path_prob[4] = self.crcmc(image, text)
...
class Reversed_DynamicInteraction_Layer(nn.Module):
    ...
    def forward(self, ref_wrd, text, image):
        ...
        # 移除: emb_lst[4], path_prob[4] = self.crcmc(ref_wrd[4], text)
